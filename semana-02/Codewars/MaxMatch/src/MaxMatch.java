...
public class MaxMatch {
    public static List<String> maxMatch(String sentence){
        ...
        //If the word it's empty return an empty list
        if(sentence.trim().isEmpty()){
            return list;
        }

        for (int i = 0; i < sentence.length(); i++){
            //We get the word with a specific part to check if exists
            String word = sentence.substring(0, sentence.length() - i);
            //If the word exist or have a length of 1 we store the word to the list
            if(Preloaded.VALID_WORDS.contains(word) || (word.length() == 1)){
                list.add(word);
                list.addAll(maxMatch(sentence.substring(word.length())));
                break;
            }
        }
        return list;
    ...
    public static void main(String[] args) {
        System.out.println(maxMatch("ewingsa"));
    }
}
...
Write maxMatch, which takes an alphanumeric, spaceless, lowercased String as input and returns a List of Strings which are all the words found, in the order they were found. All valid words are in the Set Preloaded.VALID_WORDS, , which only contains around 500 English words.
